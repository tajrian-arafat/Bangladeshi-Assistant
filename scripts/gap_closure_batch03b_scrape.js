#!/usr/bin/env node
/**
 * Batch 3B gap closure — browser-rendered BSP/BRTA official source capture.
 */
...
const OUT_DIR = path.join(__dirname, '../data/research/verification/batch-03b-brta-vehicle-gap-closure/source_snapshots');
const CHROME = process.env.CHROME_PATH || '/usr/local/bin/google-chrome';

const TARGETS = [
  { id: 'bsp_vehicle_registration', url: 'https://bsp.brta.gov.bd/vehicleRegistration/?lan=en', waitMs: 8000 },
  { id: 'bsp_tbc', url: 'https://bsp.brta.gov.bd/tbc/', waitMs: 8000 },
  { id: 'bsp_fee_calculator', url: 'https://bsp.brta.gov.bd/feeCalculator', waitMs: 12000 },
  { id: 'bsp_register', url: 'https://bsp.brta.gov.bd/register', waitMs: 8000 },
  { id: 'bsp_home', url: 'https://bsp.brta.gov.bd/bsp/?lan=en', waitMs: 8000 },
  { id: 'brta_ownership_transfer', url: 'http://brta.portal.gov.bd/pages/static-pages/6922dc6b933eb65569e10468', waitMs: 15000 },
  { id: 'brta_drc_biometric', url: 'http://brta.portal.gov.bd/pages/static-pages/6922dba6933eb65569e0b8fe', waitMs: 15000 },
  { id: 'brta_info_correction', url: 'http://brta.portal.gov.bd/pages/static-pages/6922dc03933eb65569e0df09', waitMs: 15000 },
  { id: 'brta_retro_plate', url: 'http://brta.portal.gov.bd/pages/static-pages/6922db7a933eb65569e0a505', waitMs: 15000 },
  { id: 'brta_fitness_crossref', url: 'http://brta.portal.gov.bd/pages/static-pages/6922db91933eb65569e0af12', waitMs: 15000 },
];
...
async function tryFeeCalculator(page) {
  ...
  try {
    ...
    // Probe first few option combinations without inventing amounts
    const probe = await page.evaluate(() => {
      const selects = Array.from(document.querySelectorAll('select'));
      return selects.map((s, i) => ({
        index: i,
        name: s.name || s.id || `select_${i}`,
        options: Array.from(s.options).slice(0, 8).map((o) => o.textContent?.trim()).filter(Boolean),
      }));
    });
    out.samples = probe;
